...
weaponlist = [['Short Sword', 2, 4], ['Long Sword', 4, 6], ['Blackbeards Saber', 5, 8], ['Pointed Mace', 4, 8],
              ['2 Handed Sword', 7, 12], ['Gordons Trident', 6, 10], ['Flaming Dragon Dagger', 4, 7]]
initweapon = ['Fists', 1, 1]
# armor is [0-'name',1-AC]
armorlist = [['Leather Armor', 2], ['Chain Armor', 4], ['Mail Armor', 6], ['Plate Armor', 8], ['Etherium Armor', 10],
             ['Padded Mail', 5], ['Black or Green Robe', 3], ['Horned Helmet', 2]]
initarmor = ['Linens', 1]
# monster is [0-'Name',1-monster hp,2-level,3-lowdamage,4-highdamage,5-monster ac,6-monsterxp]
monsterlist = [['Orc', 5, 1, 1, 2, 3, 100], ['Troll', 7, 2, 3, 5, 6, 200], ['Sicilian Gnome', 8, 2, 2, 8, 1, 250],
               ['M.O.U.S', 10, 2, 4, 6, 5, 400], ['Giant', 8, 2, 3, 6, 1, 750], ['Ooze', 5, 3, 2, 5, 10, 800]]
...
def SelectMonster(monsterlist, goblin, boss):
    ...
    if goblin == True:
        ...
        MonsterClass.maxhp = 20
        MonsterClass.currenthp = 20
        MonsterClass.level = 7
        MonsterClass.lowdamage = 6
        MonsterClass.highdamage = 12
        MonsterClass.ac = 10
        MonsterClass.xp = 3000
    if goblin != True and boss != True:
...
def GoblinBuying():
    ...
    vendorweapon = [['Rusted Sword', 1, 2, 10], ['Shiny Dagger', 3, 6, 20], ['Andres Fist', 4, 8, 25],
                    ['Inigos Sword', 5, 10, 35], ['Magic Sword', 12, 20, 150]]
    vendorarmor = [['Diced Leather', 1, 10], ['Chain Mail', 4, 30], ['Polished Plate', 10, 100],
                   ['Magic Armor', 20, 200]]
    inbuying = True
    while inbuying == True:
...
def PrintDashboard():
    ...
    print"-----------------------------------------------------------------------------------------------------------------------------------"
    print"\n \n"
    print"   HP: %d/%d     XP: %d      Gold: %d      Armor: %s   AC:%d     Damage: %s(%d-%d)     Level: %d     Name: %s " % (
    PlayerClass.currenthp, PlayerClass.maxhp, PlayerClass.xp, PlayerClass.gold, PlayerClass.armor, PlayerClass.ac,
    PlayerClass.weapon, PlayerClass.lowdamage, PlayerClass.highdamage, PlayerClass.level, PlayerClass.name)
    print"\n \n"
    print "Hall: %s |Rm1: %s |Rm2: %s |Rm3: %s |Rm4 chest: %s |Rm4 pass: %s |Rm5: %s |Rm6: %s |Rm6 pass: %s|Rm7: %s |Rm8: %s" % (
    RoomsClear.hallway, RoomsClear.room1, RoomsClear.room2, RoomsClear.room3, RoomsClear.room4chest,
    RoomsClear.room4pass, RoomsClear.room5, RoomsClear.room6, RoomsClear.room6pass, RoomsClear.room7, RoomsClear.room8)
    print"-----------------------------------------------------------------------------------------------------------------------------------"
...
def PlayerDamage(dammod):
    ...
    if dammod > 0:
        selfdamage = 0
        damage = int(round(float(PlayerClass.level) / float(MonsterClass.level)) * (
        random.randint(PlayerClass.lowdamage, PlayerClass.highdamage) * dammod))
        if damage == 0:
            damage = 1
    if dammod < 0:
...
def MonsterDamage(dammod):
    monsterdamage = int(round(((float(PlayerClass.level) / float(MonsterClass.level)) * (
    random.randint(MonsterClass.lowdamage, MonsterClass.highdamage) * dammod))))
    if dammod < 0:
...
def PickUpWeapon(winput, weapon):
    i = 0
    j = 0
    while winput != 'P' and winput != 'p' and winput != 'l' and winput != 'L':
        if i > 1 and i < 4:
        ...
        winput = raw_input("Enter your choice: ")
        i = i + 1
...
def PickUpArmor(ainput, armorloot):
    i = 0
    j = 0
    while ainput != 'P' and ainput != 'p' and ainput != 'l' and ainput != 'L':
        if i > 2 and i < 5:
            print"Sigh....."
        if i > 4 and i < 7:
            print"Huff..........."
        ...
        if i > 9:
            ...
            while j < 50:
                ...
                i = 0
        print" That is not a valid selection.  Please choose (P) to pickup or (L)to leave alone"
        ainput = raw_input("Enter your choice: ")
        i = i + 1
    if ainput == 'p' or ainput == 'P':
        PlayerClass.armor = armorloot[0]
        PlayerClass.ac = armorloot[1]
        ChoiceTaken()
        return
    if ainput == 'l' or ainput == 'L':
        return
...
def Battle(rolllist):
    print"\n"
    print"You are fighting a %s.  Monster Level: %d   Monster Armor: %d   Damage: %d-%d   Monster hp: %d" % (
    MonsterClass.name, MonsterClass.level, MonsterClass.ac, MonsterClass.lowdamage, MonsterClass.highdamage,
    MonsterClass.maxhp)
    while (PlayerClass.currenthp > 0) and (MonsterClass.currenthp > 0):
        ...
        if (PlayerClass.currenthp > 0) and (MonsterClass.currenthp > 0):
            print "%s HP:%d        %s HP:%d" % (
            PlayerClass.name, PlayerClass.currenthp, MonsterClass.name, MonsterClass.currenthp)
            dammod = MonsterHit()
...
def Hallway(monsterlist, rolllist):
    ...
    if RoomsClear.hallway == False:
        ...
        winput = raw_input(
            "(P)ick up the sword.  (L)eave the sword alone, I'd rather bloody my knuckles fighting with my fists : ")
        PickUpWeapon(winput, weaponlist[1])
        if winput == 'l' or winput == 'L':
            pickupsword = False
        if winput == 'P' or winput == 'p':
            pickupsword = True
...
def Room1(monsterlist, rolllist):
    PrintDashboard()
    if RoomsClear.room1 == True and  RoomsClear.room2 == True and  RoomsClear.room3 == True and  RoomsClear.room4pass == True and  RoomsClear.room5 == True and  RoomsClear.room6 == True and  RoomsClear.room7 == True and  RoomsClear.room8 == True:
        ...
        attackorrun=raw_input("(A)ttack the pirate Robert Dread.  (R)un away? :")
            ...
            while room5choice2 == 'D' or room5choice2 == 'd':
                ...
                if disarmtrap < 3:
                    ...
                    if PlayerClass.currenthp < 1:
                        print"You have died"
                        sys.exit()
                if disarmtrap > 2 and disarmtrap < 18:
                    print"You could not figure out how to disarm the trap!"
                if disarmtrap > 17:
                    print"You successfully disarmed the trap!"
                    print"You gain 100xp"
                    PlayerClass.xp = PlayerClass.xp + 100
                    RoomsClear.room5 = True
                    room5choice2 = 'Y'
                if room5choice2 != 'Y':
                    room5choice2 = raw_input("Door to the (N)orth, Door to the (E)ast, (D)isarm arrows?: ")
                    if room5choice2 == 'N' or room5choice2 == 'n':
                        Room3(monsterlist, rolllist)
                    if room5choice2 == 'E' or room5choice2 == 'e':
                        Room4(monsterlist, rolllist)
    if RoomsClear.room5 == True:
        print"The room is small, 10 feet long by 20 feet wide.  The room is almost empty except for the dozens of bloodied broken arrows laying on the floor next to gnawed bones."
        print"Would you like to open the door to the North, Open the door to the East?"
        room5choice3 = raw_input("Door to the (N)orth, Door to the (E)ast?: ")
        if room5choice3 == 'N' or room5choice3 == 'n':
            Room3(monsterlist, rolllist)
        if room5choice3 == 'E' or room5choice3 == 'e':
            Room4(monsterlist, rolllist)
